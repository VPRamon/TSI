...
export function toSkyMapViewModel(data: SkyMapData): SkyMapViewModel {
  ...
  return {
    scheduledBlocks: scheduled.map((b) => ({
      ...
      priority: b.priority,
      hoverText: `${b.block_name ? `${b.block_name}<br>` : ''}ID: ${b.original_block_id}<br>Priority: ${b.priority.toFixed(1)}`,
    })),
...
export function toTimelineViewModel(
  ...
  return {
    ...
    blocks: data.blocks.map((block, index) => ({
      ...
      hoverText: `${block.block_name ? `${block.block_name}<br>` : ''}ID: ${block.original_block_id}<br>Priority: ${block.priority.toFixed(1)}<br>Duration: ${block.requested_hours.toFixed(1)}h`,
    })),

...
use crate::io::loaders::{DarkPeriodsLoader, ScheduleLoader};
...
pub fn load_schedule(file_path: &str) -> PyResult<PyDataFrame> {
    ...
    let result = ScheduleLoader::load_from_file(&path).map_err(|e| {
        pyo3::exceptions::PyRuntimeError::new_err(format!("Failed to load schedule: {}", e))
    })?;
...
pub fn load_schedule_from_json(json_path: &str) -> PyResult<PyDataFrame> {
    ...
    let result = ScheduleLoader::load_from_json(&path).map_err(|e| {
...
pub fn load_schedule_from_json_str(json_str: &str) -> PyResult<PyDataFrame> {
    let result = ScheduleLoader::load_from_json_str(json_str).map_err(|e| {
        // Format the full error chain for better debugging
...
pub fn load_schedule_from_csv(csv_path: &str) -> PyResult<PyDataFrame> {
    ...
    let result = ScheduleLoader::load_from_csv(&path).map_err(|e| {
        pyo3::exceptions::PyRuntimeError::new_err(format!("Failed to load CSV: {}", e))
    })?;
...
pub fn load_dark_periods(file_path: &str) -> PyResult<PyDataFrame> {
    ...
    let dataframe = DarkPeriodsLoader::load_from_file(&path).map_err(|e| {
        pyo3::exceptions::PyRuntimeError::new_err(format!("Failed to load dark periods: {}", e))
    })?;

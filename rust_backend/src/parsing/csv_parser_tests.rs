...
mod csv_parser_tests {
    use crate::parsing::csv_parser::{parse_schedule_csv, parse_schedule_csv_to_blocks, dataframe_to_blocks, blocks_to_dataframe};
    ...
    fn test_parse_csv_without_coordinates() {
        ...
        assert!(result.is_ok(), "Should parse CSV without coordinates: {:?}", result.err());
        let blocks = result.unwrap();
        assert_eq!(blocks.len(), 1);
        assert!(blocks[0].coordinates.is_none(), "Coordinates should be None");
    }
    ...
    fn test_parse_csv_without_elevation() {
        ...
        assert!(result.is_ok(), "Should parse CSV without elevation: {:?}", result.err());
        let blocks = result.unwrap();
        assert_eq!(blocks.len(), 1);
    ...
    fn test_parse_csv_without_azimuth() {
        ...
        assert!(result.is_ok(), "Should parse CSV without azimuth: {:?}", result.err());
        let blocks = result.unwrap();
        assert_eq!(blocks.len(), 1);
    ...
    fn test_parse_csv_with_visibility() {
        ...
        assert!(result.is_ok(), "Should parse CSV with visibility: {:?}", result.err());
        let blocks = result.unwrap();
        assert_eq!(blocks.len(), 1);
        assert!(!blocks[0].visibility_periods.is_empty(), "Should have visibility periods");
    ...
    fn test_parse_csv_with_scheduled_period() {
        ...
        assert!(result.is_ok(), "Should parse CSV with scheduled period: {:?}", result.err());
        let blocks = result.unwrap();
        assert_eq!(blocks.len(), 1);
    ...
    fn test_parse_csv_missing_headers() {
        ...
        assert!(error_msg.contains("requestedDurationSec") || error_msg.contains("column"), 
    ...
    fn test_parse_csv_nan_priority() {
        ...
        assert!(result.is_ok(), "CSV should parse with NaN: {:?}", result.err());
        let df = result.unwrap();
    ...
    fn test_parse_csv_invalid_float() {
        ...
        assert!(result.is_ok(), "Should parse with invalid float as null: {:?}", result.err());
        let df = result.unwrap();
        assert_eq!(df.height(), 1);
    }
    ...
    fn test_blocks_to_dataframe_derived_columns() {
        ...
        assert!(result.is_ok(), "Should convert blocks to DataFrame: {:?}", result.err());
        
        let df = result.unwrap();
        
        // Check derived columns exist
        assert!(df.column("scheduled_flag").is_ok(), "Should have scheduled_flag column");
        assert!(df.column("priority_bin").is_ok(), "Should have priority_bin column");
        assert!(df.column("total_visibility_hours").is_ok(), "Should have total_visibility_hours column");
        assert!(df.column("num_visibility_periods").is_ok(), "Should have num_visibility_periods column");
        assert!(df.column("requested_hours").is_ok(), "Should have requested_hours column");
        assert!(df.column("elevation_range_deg").is_ok(), "Should have elevation_range_deg column");
        ...
        assert!(priority_bin_value.contains("high") || priority_bin_value.contains("8"), 
        ...
        assert_eq!(requested_hours.get(0), Some(1200.0 / 3600.0), "Should convert seconds to hours");
        ...
        if let Some(er) = elev_range {
            ...
            assert_eq!(range_val.unwrap(), 30.0, "Elevation range should be 90-60=30");
        }
    }
    ...
    fn test_blocks_to_dataframe_unscheduled() {
        ...
        let result = blocks_to_dataframe(&blocks);
        assert!(result.is_ok(), "Should convert unscheduled blocks: {:?}", result.err());
        
        let df = result.unwrap();
        
        let scheduled_flags = df.column("scheduled_flag").unwrap().bool().unwrap();
        assert_eq!(scheduled_flags.get(0), Some(false), "Should not be scheduled");
        
        let num_vis = df.column("num_visibility_periods").unwrap().u32().unwrap();
        assert_eq!(num_vis.get(0), Some(0), "Should have 0 visibility periods");
        
        let total_vis_hours = df.column("total_visibility_hours").unwrap().f64().unwrap();
        assert_eq!(total_vis_hours.get(0), Some(0.0), "Should have 0 visibility hours");
    }
    ...
    fn test_blocks_to_dataframe_empty() {
        ...
        let result = blocks_to_dataframe(&blocks);
        assert!(result.is_ok(), "Should handle empty blocks: {:?}", result.err());
    ...
    fn test_parse_csv_multiple_rows() {
        ...
        assert!(result.is_ok(), "Should parse multiple rows: {:?}", result.err());
        let blocks = result.unwrap();
    ...
    fn test_parse_csv_partial_coordinates() {
        ...
        assert!(result.is_ok(), "Should parse with only RA: {:?}", result.err());
        let blocks = result.unwrap();
        assert_eq!(blocks.len(), 1);
        // Without Dec, coordinates should be None
        assert!(blocks[0].coordinates.is_none(), "Coordinates should be None without both RA and Dec");
    }
    ...
    fn test_parse_csv_empty_visibility() {
        ...
        assert!(result.is_ok(), "Should parse with empty visibility: {:?}", result.err());
        let blocks = result.unwrap();
        assert_eq!(blocks.len(), 1);
        assert!(blocks[0].visibility_periods.is_empty(), "Should have empty visibility periods");
    }
    ...
    fn test_parse_csv_malformed_visibility() {
        ...
        assert!(result.is_ok(), "Should parse with malformed visibility: {:?}", result.err());
        let blocks = result.unwrap();
        assert_eq!(blocks.len(), 1);
        // Malformed visibility should result in empty vector
        assert!(blocks[0].visibility_periods.is_empty(), "Should have empty visibility periods for malformed input");
    }
    ...
    fn test_blocks_to_dataframe_with_fixed_time() {
        ...
        let result = blocks_to_dataframe(&blocks);
        assert!(result.is_ok(), "Should convert blocks with fixed_time: {:?}", result.err());

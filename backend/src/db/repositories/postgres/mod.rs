...
impl ValidationRepository for PostgresRepository {
    ...
    async fn fetch_validation_results(
        ...
        self.with_conn(move |conn| {
            let block_id_map: std::collections::HashMap<i64, (Option<String>, String)> =
            ...
            for row in rows.drain(..) {
                let (original_block_id, block_name) = block_id_map
    ...
    async fn fetch_visibility_map_data(
        ...
        self.with_conn(move |conn| {
            let rows = schedule_blocks::table
                ...
                .load::<(i64, i64, Option<String>, String, f64, i32, bool)>(conn)
                .map_err(map_diesel_error)?;

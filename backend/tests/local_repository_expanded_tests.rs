...
use std::sync::Arc;
use tsi_rust::api::{Constraints, Period, Schedule, ScheduleId, SchedulingBlock, SchedulingBlockId};
use tsi_rust::db::repositories::LocalRepository;
use tsi_rust::db::repository::{ScheduleRepository, AnalyticsRepository};
...
async fn test_concurrent_list_and_store() {
    ...
    for i in 0..20 {
        ...
        if i % 2 == 0 {
            ...
            // List
            let handle = tokio::spawn(async move {
                repo_clone.list_schedules().await
            });
...
async fn test_concurrent_analytics_population() {
    ...
    let handles: Vec<_> = schedule_ids
        ...
        .map(|schedule_id| {
            let repo_clone = Arc::clone(&repo);
            tokio::spawn(async move {
                repo_clone.populate_schedule_analytics(schedule_id).await
            })
        })
        .collect();

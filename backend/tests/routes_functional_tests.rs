...
async fn test_full_api_stack_store_to_visualization() {
    ...
    // 2. List schedules
    let schedules = services::list_schedules(&repo).await.unwrap();
    assert!(schedules.iter().any(|s| s.schedule_id == metadata.schedule_id));
    ...
    let blocks = services::get_blocks_for_schedule(&repo, metadata.schedule_id).await.unwrap();
...
async fn test_multiple_schedules_isolation() {
    ...
    let blocks1 = services::get_blocks_for_schedule(&repo, meta1.schedule_id).await.unwrap();
    let blocks2 = services::get_blocks_for_schedule(&repo, meta2.schedule_id).await.unwrap();
    let blocks3 = services::get_blocks_for_schedule(&repo, meta3.schedule_id).await.unwrap();
    ...
    let sched1 = services::get_schedule(&repo, meta1.schedule_id).await.unwrap();
    assert_eq!(sched1.name, "sched1");
    
    let sched2 = services::get_schedule(&repo, meta2.schedule_id).await.unwrap();
...
async fn test_analytics_population_on_store() {
    ...
    let metadata = services::store_schedule_with_options(&repo, &schedule, true).await.unwrap();
...
async fn test_concurrent_read_operations() {
    ...
    let handles: Vec<_> = (0..10)
        .map(|_| {
            ...
            tokio::spawn(async move {
                services::get_schedule(&repo_clone, schedule_id).await
            })
        })
        .collect();

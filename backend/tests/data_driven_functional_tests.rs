...
use tsi_rust::services::{timeline, trends, distributions, insights, sky_map, compare};
...
fn load_schedule_from_files() -> Schedule {
    let schedule_json = fs::read_to_string("/workspace/data/schedule.json")
    ...
    parse_schedule_json_str(&schedule_json, Some(&possible_periods_json), &dark_periods_json)
...
async fn test_full_workflow_load_store_validate() {
    ...
    assert!(!schedule.checksum.is_empty(), "Schedule should have checksum");
    assert!(!schedule.blocks.is_empty(), "Schedule should have blocks");
    assert!(!schedule.dark_periods.is_empty(), "Schedule should have dark periods");

    println!("Loaded schedule: {} blocks, {} dark periods", 
             schedule.blocks.len(), schedule.dark_periods.len());
    ...
    assert!(metadata.schedule_id.value() > 0, "Should have valid schedule ID");
    ...
    let validation_report = repo.fetch_validation_results(metadata.schedule_id)
        .await
        .expect("Failed to fetch validation report");

    assert_eq!(validation_report.total_blocks, schedule.blocks.len());
    println!("Validation report: {} total blocks, {} impossible, {} errors, {} warnings",
...
async fn test_full_workflow_sky_map_data() {
    ...
    println!("Stored schedule {} with {} blocks", metadata.schedule_id, schedule.blocks.len());

    // Retrieve sky map blocks directly from repository
    let blocks = repo.fetch_analytics_blocks_for_sky_map(metadata.schedule_id)
        .await
        .expect("Failed to fetch sky map blocks");

    if blocks.is_empty() {
        println!("No sky map blocks found - this is expected when blocks have no valid coordinates");
        return;
    }

    // Compute sky map data
    let sky_map = sky_map::compute_sky_map_data(blocks)
        .expect("Failed to compute sky map data");

    // Verify sky map structure
    println!("Sky map data: {} blocks, bounds: RA [{}, {}], Dec [{}, {}]",
             sky_map.blocks.len(),
             sky_map.ra_min.value(), sky_map.ra_max.value(),
             sky_map.dec_min.value(), sky_map.dec_max.value());
...
async fn test_full_workflow_trends_data() {
    ...
    let blocks = repo.fetch_analytics_blocks_for_insights(metadata.schedule_id)
        .await
    ...
    let trends_blocks: Vec<TrendsBlock> = blocks.into_iter()
        .filter_map(|b| {
    ...
    if !trends_blocks.is_empty() {
        ...
        // Verify metrics
        println!("Trends metrics: total={}, scheduled={}, rate={:.2}%",
...
async fn test_full_workflow_timeline_data() {
    ...
    let blocks = repo.fetch_schedule_timeline_blocks(metadata.schedule_id)
        .await
    ...
    assert_eq!(timeline.total_count, schedule.blocks.len(),
              "Timeline should report correct total count");
    
    println!("Timeline data: {} blocks, {} scheduled, priority range [{}, {}], {} unique months",
    ...
    for block in &timeline.blocks {
        ...
        assert!(block.scheduled_start_mjd.value() > 0.0, "Start MJD should be positive");
    }
}
...
async fn test_full_workflow_validation_report() {
    ...
    let report = repo.fetch_validation_results(metadata.schedule_id)
        .await
        .expect("Failed to retrieve validation report");

    // Verify report structure
    assert_eq!(report.total_blocks, schedule.blocks.len(),
    ...
    let categorized = report.impossible_blocks.len() + report.validation_errors.len() + report.validation_warnings.len();
    assert!(categorized <= report.total_blocks,
           "Categorized issues should not exceed total blocks");

    // Verify valid blocks calculation
    assert!(report.valid_blocks <= report.total_blocks,
...
async fn test_full_workflow_distributions_data() {
    ...
    let blocks = repo.fetch_analytics_blocks_for_distribution(metadata.schedule_id)
        .await
    ...
    let impossible_count = blocks.iter().filter(|b| b.total_visibility_hours.value() == 0.0).count();
    ...
    println!("  Priority stats: mean={:.2}, median={:.2}, min={:.2}, max={:.2}",
    ...
    assert_eq!(distributions.total_count, 
              distributions.scheduled_count + distributions.unscheduled_count + distributions.impossible_count,
...
async fn test_full_workflow_insights_data() {
    ...
    let filtered_blocks: Vec<_> = blocks.into_iter()
    ...
    let insights = insights::compute_insights_data(filtered_blocks)
        .expect("Failed to compute insights data");
    ...
    // Verify metrics
    println!("  Scheduling rate: {:.2}%", insights.metrics.scheduling_rate * 100.0);
    ...
    // Verify top lists
    println!("  Top priority observations: {}", insights.top_priority.len());
    println!("  Top visibility observations: {}", insights.top_visibility.len());
...
async fn test_large_schedule_performance() {
    ...
    println!("Loaded large schedule in {:?}: {} blocks, {} dark periods",
             load_time, schedule.blocks.len(), schedule.dark_periods.len());
    ...
    let start = std::time::Instant::now();
    let validation = repo.fetch_validation_results(metadata.schedule_id)
        .await
        .expect("Failed to fetch validation");
    let validation_time = start.elapsed();

    println!("Retrieved validation in {:?}: {}/{} blocks validated",
             validation_time, validation.total_blocks, schedule.blocks.len());
    ...
    println!("Retrieved large schedule in {:?}: {} blocks",
             retrieve_time, retrieved.blocks.len());
...
async fn test_visibility_periods_preserved() {
    ...
    let blocks_with_visibility = schedule.blocks.iter()
        .filter(|b| !b.visibility_periods.is_empty())
        .count();
    
    println!("Schedule has {} blocks with visibility periods out of {} total",
             blocks_with_visibility, schedule.blocks.len());

    assert!(blocks_with_visibility > 0, 
    ...
    let retrieved_blocks_with_visibility = retrieved.blocks.iter()
        .filter(|b| !b.visibility_periods.is_empty())
        .count();

    assert_eq!(retrieved_blocks_with_visibility, blocks_with_visibility,
    ...
    if let Some(original_block) = schedule.blocks.first() {
        if let Some(retrieved_block) = retrieved.blocks.iter()
            .find(|b| b.id == original_block.id) {
            assert_eq!(retrieved_block.visibility_periods.len(),
...
async fn test_dark_periods_preserved() {
    ...
    assert!(!schedule.dark_periods.is_empty(), "Should have dark periods");
...
async fn test_full_workflow_compare_schedules() {
    ...
    println!("Stored two schedules for comparison: {} vs {}", 
             metadata1.schedule_id, metadata2.schedule_id);

    // Fetch blocks from both schedules using repository methods
    let current_blocks = repo.fetch_compare_blocks(metadata1.schedule_id)
        .await
        .expect("Failed to fetch current schedule blocks");
    let comparison_blocks = repo.fetch_compare_blocks(metadata2.schedule_id)
        .await
    ...
    println!("  Current ({}) blocks: {}", comparison.current_name, comparison.current_blocks.len());
    println!("  Comparison ({}) blocks: {}", comparison.comparison_name, comparison.comparison_blocks.len());
    println!("  Common IDs: {}", comparison.common_ids.len());
    println!("  Only in current: {}", comparison.only_in_current.len());
    println!("  Only in comparison: {}", comparison.only_in_comparison.len());
    println!("  Scheduling changes: {}", comparison.scheduling_changes.len());

    // Verify stats
    println!("  Current stats: scheduled={}, unscheduled={}, mean_priority={:.2}",
             comparison.current_stats.scheduled_count,
             comparison.current_stats.unscheduled_count,
             comparison.current_stats.mean_priority);
    println!("  Comparison stats: scheduled={}, unscheduled={}, mean_priority={:.2}",
...
async fn test_block_data_integrity() {
    ...
    for (orig, retr) in schedule.blocks.iter().zip(retrieved.blocks.iter()) {
        ...
        // Verify priority
        assert!((orig.priority - retr.priority).abs() < 0.001, 
               "Priority mismatch for block {}", orig.id);
        
        // Verify target coordinates (target_ra and target_dec are direct fields)
        assert!((orig.target_ra.value() - retr.target_ra.value()).abs() < 0.001,
               "RA mismatch for block {}", orig.id);
        assert!((orig.target_dec.value() - retr.target_dec.value()).abs() < 0.001,
               "Dec mismatch for block {}", orig.id);
        
        // Verify scheduled period if present
        if let (Some(orig_period), Some(retr_period)) = (&orig.scheduled_period, &retr.scheduled_period) {
            assert!((orig_period.start.value() - retr_period.start.value()).abs() < 0.0001,
                   "Scheduled start mismatch for block {}", orig.id);
            assert!((orig_period.stop.value() - retr_period.stop.value()).abs() < 0.0001,
                   "Scheduled stop mismatch for block {}", orig.id);
        }
    }
...
async fn test_constraints_preserved() {
    ...
    for (orig, retr) in schedule.blocks.iter().zip(retrieved.blocks.iter()) {
        ...
        assert_eq!(orig.constraints.min_alt.value(), retr.constraints.min_alt.value(),
                  "Min altitude mismatch for block {}", orig.id);
        assert_eq!(orig.constraints.max_alt.value(), retr.constraints.max_alt.value(),
                  "Max altitude mismatch for block {}", orig.id);
        assert_eq!(orig.constraints.min_az.value(), retr.constraints.min_az.value(),
                  "Min azimuth mismatch for block {}", orig.id);
        assert_eq!(orig.constraints.max_az.value(), retr.constraints.max_az.value(),
                  "Max azimuth mismatch for block {}", orig.id);
        assert_eq!(orig.requested_duration.value(), retr.requested_duration.value(),
                  "Requested duration mismatch for block {}", orig.id);
    }
    
    println!("All constraints preserved for {} blocks", retrieved.blocks.len());
}
...
async fn test_analytics_populated_on_store() {
    ...
    let sky_map_blocks = repo.fetch_analytics_blocks_for_sky_map(metadata.schedule_id)
        .await
    ...
    let trends_insights_blocks = repo.fetch_analytics_blocks_for_insights(metadata.schedule_id)
        .await
        .expect("Trends blocks should be available after store");
    let trends_blocks: Vec<TrendsBlock> = trends_insights_blocks.into_iter()
        .filter_map(|b| {
            if b.total_visibility_hours.value() == 0.0 { return None; }
            Some(TrendsBlock {
                scheduling_block_id: b.scheduling_block_id,
                original_block_id: b.original_block_id,
                priority: b.priority,
    ...
    let timeline_blocks = repo.fetch_schedule_timeline_blocks(metadata.schedule_id)
        .await
        .expect("Timeline blocks should be available after store");
    let timeline = timeline::compute_schedule_timeline_data(timeline_blocks, schedule.dark_periods.clone());
    assert!(timeline.is_ok(), "Timeline should be computable after store");
    
    // Distribution data - fetch from repository and compute
    let dist_blocks = repo.fetch_analytics_blocks_for_distribution(metadata.schedule_id)
        .await
        .expect("Distribution blocks should be available after store");
    let impossible_count = dist_blocks.iter().filter(|b| b.total_visibility_hours.value() == 0.0).count();
    let dist = distributions::compute_distribution_data(dist_blocks, impossible_count);
    assert!(dist.is_ok(), "Distribution should be computable after store");
    
    // Insights data - fetch from repository and compute
    let insights_blocks = repo.fetch_analytics_blocks_for_insights(metadata.schedule_id)
        .await
        .expect("Insights blocks should be available after store");
    // Filter for schedulable blocks (like the insights service does)
    let schedulable_blocks: Vec<_> = insights_blocks.iter()
        .filter(|b| b.total_visibility_hours.value() > 0.0)
        .cloned()
        .collect();
    let insights = insights::compute_insights_data(schedulable_blocks);
    assert!(insights.is_ok(), "Insights should be computable after store");
    
    // Validation data
    let validation = repo.fetch_validation_results(metadata.schedule_id).await;
    assert!(validation.is_ok(), "Validation should be available after store");

    println!("All analytics successfully populated for schedule {}", metadata.schedule_id);
}
...
async fn test_empty_schedule_handling() {
    ...
    assert!(retrieved.blocks.is_empty(), "Retrieved schedule should have no blocks");
...
async fn test_schedule_with_extreme_values() {
    ...
    if !trends_blocks.is_empty() {
        ...
        assert!(trends.is_ok(), "Trends should handle extreme parameter values");
    }
